...
/// Starts Apache with the generated configuration.
///
/// The returned record is what `lambo status` and `lambo down` use later. The
/// caller is responsible for verifying that the port answers - see
/// [`crate::session`].
pub fn start(paths: &Paths, plan: &Plan, os: Os) -> Result<ServiceRecord> {
    let config_file = write_config(paths, plan, os)?;
    let log = crate::logs::apache(paths);
    if let Some(parent) = log.parent() {
        fs::create_dir_all(parent).map_err(|source| Error::io(parent, source))?;
    }

    let spec = command_spec(plan, &config_file, &log);
    let child = process::spawn(&spec, os)?;
    let pid = child.id();
    // The handle is dropped on purpose: Apache keeps running because it was
    // started detached, and Lambo tracks it by PID.
    drop(child);

    // Read the identity now, while this PID is unambiguously the process just
    // started. `lambo down` compares against it before signalling anything.
    // Settled rather than immediate: see [`process::identity_settled`].
    ...
    let mut record = ServiceRecord::new(names::APACHE, pid, spec.render())
        .with_port(plan.port)
        .with_log(log);
...
/// Stops Apache, gracefully when possible.
pub fn stop(paths: &Paths, apache: &Apache, record: &ServiceRecord, os: Os) -> Result<StopOutcome> {
    let graceful = ProcessSpec::new(&apache.executable, "apache")
        .arg("-k")
        .arg("stop")
        .arg("-f")
        .arg(paths.apache_config_file().display().to_string());
...
pub fn graceful_timeout() -> std::time::Duration {
...
mod tests {
    ...
    #[test]
    fn a_service_record_carrying_the_apache_pid_can_be_stopped() {
        // Spawning and stopping a long-lived child is covered in
        // `process::tests`; what is verified here is the record Lambo persists,
        // because `lambo down` and `lambo status` act on it later.
        let temp = TempDir::new();
        let paths = temp.home();
        let record = ServiceRecord::new(names::APACHE, 4242, "httpd -f httpd.conf")
            .with_port(8080)
            .with_log(crate::logs::apache(&paths));

        assert_eq!(record.name, names::APACHE);
        assert_eq!(record.pid, 4242);
        assert_eq!(record.port, Some(8080));
        assert!(record.log.as_ref().unwrap().ends_with("httpd.log"));
        assert!(
            record.command.contains("-f"),
            "the command must name the config file"
        );
        assert!(
            !record.is_alive(Os::host()),
            "a PID nobody started is not alive"
        );
    }
